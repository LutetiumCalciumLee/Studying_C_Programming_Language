...
# Chapter 13. LECTURE NOTE on PROGRAMMING in C
...
본 문서는 C 언어의 역사, 구조, 문법, 핵심 기능, 실용적 프로그래밍 개념을 포괄적으로 소개하는 자료임. 이론과 실습을 모두 이해하고자 하는 초급 및 중급 학습자를 위한 내용임.
...
- **컴파일과 실행:**  
  소스 코드를 텍스트 에디터로 작성해 `.c` 확장자로 저장한 뒤, C 컴파일러로 컴파일하고 실행 파일을 생성함. 컴파일 단계에서 문법 오류를 확인하며, 실행 단계에서 런타임 또는 논리 오류가 발생할 수 있음.
- **문자 집합, 식별자, 키워드, 상수, 변수:**  
  C 언어는 정의된 문자 집합, 예약어, 식별자 명명 규칙을 사용함. 변수는 반드시 타입을 선언한 후 사용함.
- **식과 연산자:**  
  산술, 관계, 논리, 대입, 증감, 조건, 비트, 특수 연산자 등 다양한 연산자를 지원하며, 우선순위와 결합 규칙에 따라 식을 평가함.
- **제어 구조:**  
  조건문(`if`, `if-else`, 중첩 if, `switch-case`)과 반복문(`while`, `do-while`, `for`, `break`, `continue`)을 포함함.
- **배열:**  
  단일 및 다차원 배열을 지원하며, 데이터 집합을 저장함. 배열은 0부터 인덱싱하며, 선언 시 크기를 지정함.
- **포인터:**  
  포인터는 메모리 주소를 저장하는 변수임. 포인터 선언, 산술 연산, 함수 전달, 배열과의 관계를 다룸.
- **함수:**  
  함수는 코드의 모듈화와 재사용성을 높임. 함수 선언, 정의, 호출, 매개변수 전달(값/참조), 재귀를 설명함.
- **구조체와 공용체:**  
  구조체는 서로 다른 타입의 변수를 하나의 이름으로 묶는 기능임. 공용체는 모든 멤버가 동일한 메모리 공간을 공유함.
- **동적 메모리 할당:**  
  `malloc()`, `calloc()`, `realloc()`, `free()` 함수로 실행 중 메모리를 할당 및 관리함.
- **파일 처리:**  
  파일 입출력을 위한 함수로 파일 열기, 읽기, 쓰기, 닫기 기능을 제공함.

## 실용적 측면

- 문법 설명, 코드 예시, 컴파일 및 실행 단계별 안내를 포함함.
- 문법, 런타임, 논리 오류 등 일반적인 오류와 디버깅 팁을 제시함.
- 명확한 명명, 모듈화, 올바른 메모리 관리 등 좋은 프로그래밍 습관을 강조함.
